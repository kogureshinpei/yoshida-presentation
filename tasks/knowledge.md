...
**背景**: 発表中にすぐ各ページへ飛べることが重要 → クイックリンクハブに刷新

**上部: クイックリンクセクション**（ページ内コンポーネント）
- メインページ4リンク（トップ/農場一覧/企業一覧/登録フォーム）
- 農場詳細6ページへの直リンク（トライアル技術名バッジ付き）
- AgTech企業詳細6ページへの直リンク（ロゴイニシャル + カテゴリ表示）
- このページ内デモへのスクロールボタン（`jumpToDemo()` でタブ切替+スクロール）

**下部: インタラクティブデモセクション**（`id="demos"` でアンカー指定）
1. **学生シフト登録フロー** — 4ステップのインタラクティブデモ
2. **農家ダッシュボード** — 農場別シフト充填状況・登録学生一覧
3. **企業トライアル状況** — 農場 × AgTech企業のマッチング可視化

**ファイル内ヘルパーコンポーネント**
- `QuickCard` — `{ href, label, desc, color, Icon }` を受け取るリンクカード
- `SectionLabel` — カラー左ボーダー付きセクションヘッダー
- `jumpToDemo(tab)` — タブ切替後50ms遅延でdemos領域にスクロール
...
- `@phosphor-icons/react` で存在しないアイコン名を使わない（ビルドエラー）  
  - NG例: `BrainCircuit`（存在しない）、`WindmillIcon`（`Windmill` が正しい）  
  - 不明なアイコン名は `node_modules/@phosphor-icons/react/dist/index.d.ts` を grep して確認する
