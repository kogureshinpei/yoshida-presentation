...
/* ────────────────────────────── quick link data ─────────────────── */
const MAIN_LINKS = [
  { href: "/",            label: "トップページ",        desc: "サービス概要・3者へのCTA",  color: "#1B4332", Icon: House },
  { href: "/for-students", label: "農場一覧（学生向け）", desc: "マップ・フィルター付き一覧", color: "#2D6A4F", Icon: Plant },
  { href: "/for-farmers",  label: "AgTech企業一覧",      desc: "カテゴリ別フィルター・モーダル", color: "#40916C", Icon: Buildings },
  { href: "/register",     label: "参加登録フォーム",     desc: "学生・農家・企業の3タブ",   color: "#C0392B", Icon: NotePencil },
];

const FARM_LINKS = [
  { id: "farm-001", label: "田中農園",              sub: "北海道 · りんご・ぶどう",     badge: "AI + 農薬トライアル" },
  { id: "farm-002", label: "山本いちご農家",        sub: "栃木県 · いちご",             badge: "ロボット + IoTトライアル" },
  { id: "farm-003", label: "GREEN ROOTS 有機農場",  sub: "長野県 · 有機野菜",           badge: "バイオ3社トライアル" },
  { id: "farm-004", label: "鈴木水田農家",          sub: "新潟県 · コシヒカリ",         badge: "ドローン + 衛星AI" },
  { id: "farm-005", label: "瀬戸内みかん農園",      sub: "愛媛県 · みかん",             badge: "農薬ゼロ達成" },
  { id: "farm-006", label: "九州野菜ファーム",      sub: "熊本県 · トマト・ピーマン",   badge: "IoT + ロボット検討" },
];

const COMPANY_LINKS = [
  { id: "co-006", label: "ロボアグリ",           sub: "ロボット", desc: "自律収穫・除草ロボで農業を完全自動化", color: "#1B4332" },
  { id: "co-001", label: "AgriAI株式会社",       sub: "AI",       desc: "病害虫AI診断・収穫量予測モデル",       color: "#2D6A4F" },
  { id: "co-002", label: "バイオネクスト",       sub: "バイオ",   desc: "土壌微生物製剤で農薬51%削減",         color: "#52B788" },
  { id: "co-008", label: "BioSoil Lab",          sub: "バイオ",   desc: "ゲノム最適化有機資材・遺伝子科学",    color: "#40916C" },
  { id: "co-003", label: "SkyFarm Technologies", sub: "ドローン", desc: "農薬散布ドローン・飛行代行38都道府県", color: "#1B4332" },
  { id: "co-005", label: "アグロファーマシー",   sub: "農薬",     desc: "農薬ゼロ生物農薬・フェロモン交信撹乱", color: "#74C69D" },
];
...
function getMockStudents(filled: number, seed: number) {
...
}: { href: string; label: string; desc: string; color: string; Icon?: React.ElementType }) {
  return (
    <Link
      href={href}
      className="flex items-center gap-3 bg-white rounded-xl px-4 py-3.5 border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-150 group"
    >
      {Icon ? (
        <span className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-white" style={{ backgroundColor: color }}>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-gray-900 leading-snug truncate">{label}</p>
        <p className="text-xs text-gray-400 truncate mt-0.5">{desc}</p>
      </div>
      <ArrowRight size={14} className="text-gray-300 group-hover:text-[#2D6A4F] flex-shrink-0 transition-colors" />
    </Link>
  );
}
...
/* ════════════════════════════════════════════════════════════════════
   SECTION HEADER
══════════════════════════════════════════════════════════════════════ */
function SectionLabel({ label, color = "#C0392B" }: { label: string; color?: string }) {
  return (
  ...
  return (
    <div className="space-y-6">
            ...
            <div key={label} className="flex items-center">
              <div className="flex flex-col items-center gap-1">
                <div className={["w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300",
                  done ? "bg-[#C0392B] text-white" : active ? "bg-[#2D6A4F] text-white ring-4 ring-[#2D6A4F]/20" : "bg-gray-200 text-gray-400"].join(" ")}>
                  {done ? <CheckCircle size={16} weight="fill" /> : n}
                </div>
                <span className={`text-[10px] font-medium ${active ? "text-[#2D6A4F]" : "text-gray-400"}`}>{label}</span>
              </div>
              {i < STEPS.length - 1 && (
            ...
            <div className="space-y-3">
              {DEMO_SHIFTS.map((slot) => (
                <button key={slot.id} type="button" onClick={() => setSelectedSlot(slot.id)}
                  className={["w-full flex items-center justify-between p-4 rounded-xl border-2 text-left transition-all duration-150",
                    selectedSlot === slot.id ? "border-[#2D6A4F] bg-[#2D6A4F]/5" : "border-gray-200 hover:border-[#74C69D]"].join(" ")}>
                  <div className="flex items-center gap-3">
                    <CalendarCheck size={20} weight={selectedSlot === slot.id ? "fill" : "regular"}
                  ...
                  {selectedSlot === slot.id && <CheckCircle size={20} weight="fill" className="text-[#2D6A4F] flex-shrink-0" />}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="p-6">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: "シフト",    value: selectedSlot !== null ? DEMO_SHIFTS[selectedSlot].label : "6/1（月）08:00〜15:00" },
                { label: "作業内容",  value: "りんご摘果・袋かけ作業" },
                { label: "氏名",      value: "山本 花子" },
                { label: "大学・学年", value: "東京農業大学 農学部 3年" },
                { label: "メモ",      value: "農業は未経験ですが体力には自信があります！" },
              ].map(({ label, value }) => (
      ...
      <div className="flex items-center justify-between">
        ...
        {step < 4 ? (
          <button type="button" onClick={() => { if (step === 2 && selectedSlot === null) setSelectedSlot(0); setStep(Math.min(4, step + 1)); }}
          ...
          <button type="button" onClick={() => { setStep(1); setSelectedSlot(null); }}
...
function FarmDashboard() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <label className="text-sm font-semibold text-gray-600 whitespace-nowrap">農場:</label>
        <select value={farmId} onChange={(e) => setFarmId(e.target.value)}
          ...
          {farms.map((f) => <option key={f.id} value={f.id}>{f.name}（{f.prefecture}）</option>)}
        </select>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "登録学生数",  value: `${totalFilled}名`,  color: "text-[#2D6A4F]" },
          { label: "空き枠",      value: `${totalCapacity - totalFilled}枠`, color: totalCapacity - totalFilled === 0 ? "text-[#C0392B]" : "text-amber-600" },
      ...
      <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
        <div className="flex justify-between text-xs text-gray-500 mb-1.5">
          <span>充填状況</span><span>{totalFilled}/{totalCapacity} 名</span>
        </div>
        <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full rounded-full transition-all duration-500"
            style={{ width: `${fillRate}%`, backgroundColor: fillRate >= 80 ? "#2D6A4F" : fillRate >= 50 ? "#52B788" : "#C0392B" }} />
        </div>
      </div>
            ...
            return (
              <div key={i} className="px-5 py-4">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <span className="text-xs font-semibold text-gray-500 mr-2">{slot.dayOfWeek} {slot.date}</span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!full && <span className="text-[10px] text-[#C0392B] font-semibold">残{slot.capacity - slot.filled}枠</span>}
                  </div>
                </div>
                <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden mb-2">
                  <div className="h-full rounded-full" style={{ width: `${(slot.filled / slot.capacity) * 100}%`, backgroundColor: full ? "#52B788" : "#2D6A4F" }} />
                </div>
                {students.length > 0 ? (
                  <div className="flex flex-wrap gap-1">
                    {students.map((name, j) => (
                      <span key={j} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#F8F4EF] text-[11px] text-gray-600 border border-gray-200">
                        <UserCircle size={11} weight="fill" className="text-[#52B788]" />{name}
                      </span>
                    ))}
                  </div>
                ) : (
...
function CompanyTrials() {
  ...
  const [farmId, setFarmId] = useState(farms[0].id);
  const farm = farms.find((f) => f.id === farmId) ?? farms[0];
  const trials = ((trialMatchesData as TrialMatch[]).find((m) => m.farmId === farmId)?.trials) ?? [];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        {trials.length === 0 ? (
          ...
          <>
            <span className="text-sm text-gray-600">{farm.name} では <strong>{trials.length}社</strong> が技術連携中</span>
            {(["トライアル中", "導入完了", "検討中"] as const).map((s) => {
              const n = trials.filter((t) => t.status === s).length;
              if (!n) return null;
              const st = STATUS_STYLE[s];
              return <span key={s} className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border ${st.bg} ${st.text} ${st.border}`}>{s} {n}社</span>;
            })}
          </>
        )}
      </div>
          ...
          return (
            <div key={trial.companyId} className={`bg-white rounded-2xl border ${s.border} shadow-sm overflow-hidden`}>
              <div className={`px-5 py-3 flex items-center justify-between ${s.bg}`}>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white text-sm font-bold select-none flex-shrink-0"
                    style={{ backgroundColor: company.logoColor }}>{initials}</div>
                  <div>
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold border ${s.bg} ${s.text} ${s.border}`}>{trial.status}</span>
              </div>
              ...
              <div className="px-5 pb-4">
                <Link href={`/for-farmers/companies/${company.id}`}
...
const DEMO_TABS: { id: Tab; label: string; Icon: typeof GraduationCap }[] = [
  { id: "student",   label: "学生シフト登録フロー", Icon: GraduationCap },
  { id: "dashboard", label: "農家ダッシュボード",   Icon: ChartBar },
  { id: "trial",     label: "企業トライアル状況",   Icon: Buildings },
];

export default function PresentationPage() {
  const [tab, setTab] = useState<Tab>("student");

  const jumpToDemo = (t: Tab) => {
    setTab(t);
    setTimeout(() => {
    ...
    <main className="min-h-screen bg-[#F8F4EF]">
      ...
      <div className="bg-[#1B4332] pt-10 pb-12 px-4">
        <div className="max-w-3xl mx-auto">
          ...
          <h1 className="font-shippori text-3xl font-bold text-white mb-2">プレゼンテーションハブ</h1>
          <p className="text-white/70 text-sm">各ページへのクイックリンクと、インタラクティブデモをまとめています</p>
        </div>
      </div>

      <div className="max-w-3xl mx-auto px-4 -mt-4 pb-24 space-y-8">

        {/* ─────────────────────────────────────────
            クイックリンク
        ───────────────────────────────────────── */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-7">
          <h2 className="font-shippori text-lg font-bold text-gray-900 flex items-center gap-2">
            クイックリンク
            <span className="text-xs font-normal text-gray-400">— タップで各ページへ移動</span>
          </h2>

          {/* メインページ */}
          <div>
            <SectionLabel label="メインページ" color="#1B4332" />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {MAIN_LINKS.map((l) => (
                <QuickCard key={l.href} href={l.href} label={l.label} desc={l.desc} color={l.color} Icon={l.Icon} />
              ))}
            </div>
          </div>
          ...
          {/* 農場詳細 */}
          <div>
            <SectionLabel label="農場詳細ページ（AgTechトライアル情報付き）" color="#2D6A4F" />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {FARM_LINKS.map((f) => (
                <Link key={f.id} href={`/for-students/farms/${f.id}`}
                  className="flex items-start gap-3 bg-white rounded-xl px-4 py-3.5 border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-150 group">
                  <MapPin size={16} weight="fill" className="text-[#2D6A4F] flex-shrink-0 mt-0.5" />
                  <div className="flex-1 min-w-0">
                  ...
                  <ArrowRight size={14} className="text-gray-300 group-hover:text-[#2D6A4F] flex-shrink-0 mt-1 transition-colors" />
                </Link>
              ))}
            </div>
          </div>
          ...
          {/* 企業詳細 */}
          <div>
            <SectionLabel label="AgTech企業詳細ページ（製品・導入事例付き）" color="#40916C" />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {COMPANY_LINKS.map((c) => (
                <Link key={c.id} href={`/for-farmers/companies/${c.id}`}
                  className="flex items-start gap-3 bg-white rounded-xl px-4 py-3.5 border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-150 group">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0 select-none"
                    style={{ backgroundColor: c.color }}>
                    {c.label.replace(/株式会社|合同会社|有限会社/g, "").slice(0, 2)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <p className="text-sm font-semibold text-gray-900 leading-snug truncate">{c.label}</p>
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 flex-shrink-0">{c.sub}</span>
                    </div>
          ...
          {/* デモセクション */}
          <div>
            <SectionLabel label="このページ内のインタラクティブデモ" color="#C0392B" />
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {DEMO_TABS.map(({ id, label, Icon }) => (
                <button key={id} type="button" onClick={() => jumpToDemo(id)}
                  className="flex items-center gap-2.5 bg-white rounded-xl px-4 py-3.5 border-2 border-[#C0392B]/30 hover:border-[#C0392B] hover:shadow-md hover:-translate-y-0.5 transition-all duration-150 group text-left">
                  <Icon size={18} weight="fill" className="text-[#C0392B] flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-gray-800 leading-snug">{label}</p>
                    <p className="text-[10px] text-gray-400 mt-0.5">↓ 下にスクロール</p>
                  </div>
                </button>
              ))}
            </div>
        ...
        {/* ─────────────────────────────────────────
            インタラクティブデモ
        ───────────────────────────────────────── */}
        <div id="demos" className="scroll-mt-20 space-y-5">
          <div className="flex items-center gap-2">
            <span className="w-1 h-5 rounded-full bg-[#C0392B]" />
            <h2 className="font-shippori text-lg font-bold text-gray-900">インタラクティブデモ</h2>
          </div>
            ...
            {DEMO_TABS.map(({ id, label, Icon }) => (
              <button key={id} type="button" onClick={() => setTab(id)}
                className={["rounded-2xl p-3 text-left transition-all duration-200 border-2",
                  tab === id ? "bg-white border-[#C0392B] shadow-md" : "bg-white/60 border-transparent hover:bg-white hover:border-gray-200"].join(" ")}>
                <Icon size={18} weight={tab === id ? "fill" : "regular"}
                  className={tab === id ? "text-[#C0392B] mb-1" : "text-gray-400 mb-1"} />
                <p className={`text-xs font-bold leading-snug ${tab === id ? "text-gray-900" : "text-gray-500"}`}>{label}</p>
              </button>
            ))}
          </div>
